...
class CoTTA(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        self.feature_extractor_src = nn.DataParallel(self.feature_extractor_src)
    ...
    def forward_and_adapt(self, x):
        ...
        if self.CP_methods == "None":
            ...
            loss = self.softmax_entropy(outputs, outputs_ema).mean(0) + self.lamda1 * loss_certain_prototype + self.lamda2 * loss_certain
        loss.backward()
        self.optimizer.step()
        ...
        return 0.2 * outputs + 0.8 * outputs_ema, losses, stu_pred_set
...
def init_cal_from_source_test(dataset='cifar10',data_root_dir='./data'):
    ...
    if dataset in ["cifar10", "cifar10_c"]:
        ...
        # for class_label in range(1000):
            # class_indices = torch.where(torch.tensor(test_data.targets) == class_label)[0]
            # selected_indices.extend(class_indices[torch.randperm(len(class_indices))[:1]])
        selected_indices.extend(torch.randperm(len(test_data))[:500])
